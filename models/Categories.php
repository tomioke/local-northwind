...
class Categories extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // CategoryName
        $this->CategoryName = new DbField('categories', 'categories', 'x_CategoryName', 'CategoryName', '`CategoryName`', '`CategoryName`', 200, 255, -1, false, '`CategoryName`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->CategoryName->Sortable = true; // Allow sort
        $this->CategoryName->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->CategoryName->Param, "CustomMsg");
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->CategoryID->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->CategoryID->CurrentValue);
        } else {
