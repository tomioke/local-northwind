...
class OrderDetailsEdit extends OrderDetails
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("order_detail_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->order_detail_id->setQueryStringValue($keyValue);
                $this->order_detail_id->setOldValue($this->order_detail_id->QueryStringValue);
            } elseif (Post("order_detail_id") !== null) {
                $this->order_detail_id->setFormValue(Post("order_detail_id"));
                $this->order_detail_id->setOldValue($this->order_detail_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->order_detail_id->IsDetailKey) {
