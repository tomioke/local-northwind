...
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    fordersedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fordersedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Multi-Page
    fordersedit.multiPage = new ew.MultiPage("fordersedit");

    // Dynamic selection lists
    fordersedit.lists.CustomerID = <?= $Page->CustomerID->toClientList($Page) ?>;
    fordersedit.lists.EmployeeID = <?= $Page->EmployeeID->toClientList($Page) ?>;
    fordersedit.lists.ShipperID = <?= $Page->ShipperID->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fordersedit" id="fordersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="orders" data-field="x_OrderID" data-hidden="1" data-page="1" name="x_OrderID" id="x_OrderID" value="<?= HtmlEncode($Page->OrderID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
