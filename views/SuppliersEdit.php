...
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    fsuppliersedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsuppliersedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fsuppliersedit" id="fsuppliersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="suppliers" data-field="x_SupplierID" data-hidden="1" name="o_SupplierID" id="o_SupplierID" value="<?= HtmlEncode($Page->SupplierID->OldValue ?? $Page->SupplierID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
