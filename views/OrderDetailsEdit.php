...
var forder_detailsedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    forder_detailsedit = currentForm = new ew.Form("forder_detailsedit", "edit");
    ...
    // Form_CustomValidate
    forder_detailsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    forder_detailsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    forder_detailsedit.lists.ProductID = <?= $Page->ProductID->toClientList($Page) ?>;
    loadjs.done("forder_detailsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="forder_detailsedit" id="forder_detailsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->order_detail_id->Visible) { // order_detail_id ?>
    <div id="r_order_detail_id" class="form-group row">
        <label id="elh_order_details_order_detail_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->order_detail_id->caption() ?><?= $Page->order_detail_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->order_detail_id->cellAttributes() ?>>
<span id="el_order_details_order_detail_id">
<span<?= $Page->order_detail_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->order_detail_id->getDisplayValue($Page->order_detail_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="order_details" data-field="x_order_detail_id" data-hidden="1" name="x_order_detail_id" id="x_order_detail_id" value="<?= HtmlEncode($Page->order_detail_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
