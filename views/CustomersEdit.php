...
loadjs.ready("head", function () {
    ...
    fcustomersedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fcustomersedit" id="fcustomersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="customers" data-field="x_CustomerID" data-hidden="1" name="o_CustomerID" id="o_CustomerID" value="<?= HtmlEncode($Page->CustomerID->OldValue ?? $Page->CustomerID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
